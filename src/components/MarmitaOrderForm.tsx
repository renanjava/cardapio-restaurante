...
import { Check, ShoppingCart, AlertCircle, Ban, Calendar, Info } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Checkbox } from '@/components/ui/checkbox';
import { RadioGroup, RadioGroupItem } from '@/components/ui/radio-group';
import { useCart, CartItem } from '@/contexts/CartContext';
import { marmitaSizes, weeklyMenu, dayNames, dayDisplayNames, MarmitaSize, MeatOption } from '@/data/menuData';
...
export function MarmitaOrderForm({ editingItem, onComplete }: MarmitaOrderFormProps) {
  ...
  const [selectedSize, setSelectedSize] = useState<MarmitaSize | null>(initialSize || null);
  ...
  const toggleItem = (itemId: string) => {
    if (isSaturday && dayMenu.beansOnlyOne) {
      if (itemId === 'feijao-preto' || itemId === 'feijao-carioca') {
        const otherId = itemId === 'feijao-preto' ? 'feijao-carioca' : 'feijao-preto';
  ...
  const calculateExtraCharge = () => {
    ...
    if (selectedSize.id === 'mini' && selectedMeat.extraForMini && selectedMeat.extraPrice) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="text-center py-12 px-6 bg-card rounded-3xl shadow-soft max-w-sm animate-fade-in">
          ...
          <h3 className="font-display text-2xl font-bold text-foreground mb-2">Fechado aos Domingos</h3>
    ...
    <>
      <div className="flex-1 overflow-y-auto pb-32 md:pb-8">
        <div className="container mx-auto px-4 py-4 md:py-6 max-w-2xl">
            ...
            <div className="space-y-2 mb-4">
              ...
              {isSaturday && (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-primary/10 border border-primary/20 text-sm">
                  ...
                  <span>Aos sábados, escolha apenas <strong>um tipo de feijão</strong> e entregas custam <strong>R$ 2,00</strong></span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Size Selection */}
          <div className="mb-6">
            <h3 className="font-display text-lg font-bold mb-3 text-foreground">Escolha o Tamanho</h3>
              ...
              {marmitaSizes.map((size) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-primary font-bold text-lg">R$ {size.price},00</span>
                  </div>
                </button>
              ))}
            </div>
              ...
              {dayMenu.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-sm font-medium ${isChecked ? 'text-foreground' : 'text-muted-foreground line-through'}`}>
                      {item.name}
                    </span>
                  </button>
                );
          ...
          {/* Meat Selection */}
          <div className="mb-6">
            <h3 className="font-display text-lg font-bold mb-3 text-foreground">Escolha a Carne</h3>
            <div className="space-y-2">
              {dayMenu.meats.map((meat) => {
                const showExtra = meat.extraForMini && meat.extraPrice && selectedSize?.id === 'mini';
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <span className={`font-semibold ${showExtra ? 'text-primary' : 'text-foreground'}`}>
                        {meat.name}
                      </span>
                      {showExtra && (
